        ...
        {products.slice(0, 4).map((p) => (
          <div key={p.id} className="rounded-lg border border-line bg-surface p-3">
            {p.imageUrls[0] ? (
              ...
              <div className="mb-2 h-24 w-full rounded bg-surface-2" />
            )}
            <p className="truncate text-xs text-ink" title={p.name}>{p.name}</p>
            <p className="text-xs text-subtle">R{p.price.toFixed(2)} · {p.salesRank ?? 0} sold</p>
          </div>
        ))}
      </div>
    </section>
...
export default async function ProductsDashboardPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-8 p-8 text-ink">
      ...
      {!productsRes.ok && (
        <p className="rounded-lg border border-red-900 bg-red-950/40 p-3 text-sm text-danger">
