  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {months.map((m) => (
          <div key={m} className="rounded-lg border border-zinc-800 bg-zinc-900 p-3 text-sm">
            <p className="font-medium">Month {m}</p>
            <p className="text-xs text-zinc-500">{monthProgressLabel(plan.monthProgress[m])}</p>
          </div>
        ))}
      </div>
      ...
      {months.map((m) => {
        ...
        return (
          <section key={m} className="space-y-3">
            <h2 className="text-sm font-medium text-zinc-300">Month {m}</h2>
            {sections.map((s) => (
    ...
    <div className="space-y-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          {section.hashtagStrategy && <p className="text-xs text-zinc-500">#: {section.hashtagStrategy}</p>}
        </div>
        <button
          disabled={pending}
...
function ItemRow({
  ...
  const statusColor =
    item.status === 'linked_to_content_item' ? 'text-green-400'
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-950 p-3 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="font-mono text-xs uppercase text-indigo-400">{item.format}</span>
        <span className={`text-xs ${statusColor}`}>{item.status}</span>
      </div>
      {!editing ? (
        <>
          <p className="mt-1 text-zinc-200">{item.hook}</p>
          <p className="mt-1 whitespace-pre-wrap text-xs text-zinc-400">{item.fullScript}</p>
        </>
      ) : (
        <div className="mt-2 space-y-2">
          <input className="w-full rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-sm" value={hook} onChange={(e) => setHook(e.target.value)} placeholder="hook" />
          <textarea className="w-full rounded border border-zinc-700 bg-zinc-900 px-2 py-1 text-xs" rows={3} value={script} onChange={(e) => setScript(e.target.value)} placeholder="full script" />
          <button
            disabled={pending}
            onClick={() => { onRun(() => editItemScript(planId, item.id, script, hook)); setEditing(false); }}
            className="rounded bg-zinc-50 px-2 py-1 text-xs font-medium text-zinc-950 hover:bg-zinc-200 disabled:opacity-50"
          >
            Save
          </button>
        </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-zinc-500">
        ...
        <span className="flex items-center gap-1">
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="rounded border border-zinc-700 bg-zinc-900 px-1 py-0.5 text-xs" />
          <button disabled={pending || !date} onClick={() => onRun(() => reorderItem(planId, item.id, date))} className="hover:text-zinc-300 disabled:opacity-50">move</button>
        </span>
        <button onClick={() => setEditing((e) => !e)} className="hover:text-zinc-300">{editing ? 'cancel' : 'edit'}</button>
        {item.linkedContentItemId && (
          <Link href="/dashboard/approvals" className="text-indigo-400 hover:underline">→ content #{item.linkedContentItemId}</Link>
        )}
      </div>
    </div>
  );
